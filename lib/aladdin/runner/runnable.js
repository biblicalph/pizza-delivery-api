...
const createAfterAllHooksRunner = app => {
  ...
  return async () => {
    ...
    for (let i = afterAllHooks - 1; i >= 0; --i) {
 ...
 * @return {Function}
 */
const createLogger = color => title => console.log(`${color}%s\x1b[0m`, title)
/**
...
const logSuccess = title => {
  const log = createLogger('\x1b[32m')
  log(`✓ ${title}`)
}
/**
...
const logFailure = title => {
  const log = createLogger('\x1b[31m')
